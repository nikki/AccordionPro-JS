...
class accordion_pro {
	...
	public function update_accordion() {
		...
		// Check if were updating or adding a new post
		if ($clean['id']) {
			$post['ID'] = $clean['id'];
	...
	public function update_accordionCache($accordion) {
		...
		if (!empty($accordion['acc_content']['content']) && is_array($accordion['acc_content']['content'])) {
			foreach($accordion['acc_content']['content'] as $key => $content) {
				$accordion['post_content'] .= '<li><h2><span>'.esc_html($accordion['acc_content']['content_title'][$key]).'</span></h2><div>'.wp_kses($content, $allowedextratags);
		...
		// If their are accordion options which are not default, add them in.
		foreach ($this->jQueryOptions as $key => $default) {
